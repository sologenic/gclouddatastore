...
func (l *propertyLoader) loadOneElement(codec fields.List, structValue reflect.Value, p Property, prev map[string]struct{}) string {
	...
	for len(fieldNames) > 0 {
		...
		// If we never found a matching field in the codec, skip this property
		// (partial object support - allow extra fields from datastore).
		if field == nil {
			return "" // Return empty string to indicate skip (no error)
		}

		v = initField(structValue, field.Index)
		if !v.IsValid() {
			return "" // Skip invalid fields (partial object support)
		}
		if !v.CanSet() {
...
// loadEntityToStructWithUnparsed loads an entity into a struct and tracks unparsed properties.
// Returns the unparsed properties map where key is property name and value is the type name.
func loadEntityToStructWithUnparsed(dst interface{}, ent *Entity) (map[string]string, error) {
	pls, err := newStructPLS(dst)
	if err != nil {
		return nil, err
	}
	...
	// Load properties and track unparsed ones.
	...
	for _, p := range ent.Properties {
		// Check if field exists in struct before attempting to load
		// For nested fields, check all possible parent paths
		fieldNames := strings.Split(p.Name, ".")
		fieldExists := false
		for i := len(fieldNames); i > 0; i-- {
			parent := strings.Join(fieldNames[:i], ".")
			if pls.codec.Match(parent) != nil {
				fieldExists = true
				break
			}
		}
		...
		if !fieldExists {
			...
			unparsed[p.Name] = getPropertyTypeName(p.Value)
		} else if errStr != "" {
			// Field exists but there was an error (type mismatch)
			fieldName, errReason = p.Name, errStr
		}
...
// getPropertyTypeName returns a string representation of the property value's type.
func getPropertyTypeName(v interface{}) string {
	if v == nil {
		return "nil"
	}
	switch v.(type) {
	...
	case []byte:
		return "[]byte"
	case []interface{}:
		// For arrays, return the type of the first element if available
		if arr := v.([]interface{}); len(arr) > 0 {
			return "[]" + getPropertyTypeName(arr[0])
		}
		return "[]interface{}"
	default:
