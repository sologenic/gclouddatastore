...
// TestPartialObjectParsing tests loading entities with extra fields that don't exist in the struct.
// This tests the new partial object parsing feature.
func TestPartialObjectParsing(t *testing.T) {
	// Define a struct with only a subset of fields
	...
	testCases := []struct {
		...
		want        interface{}
		wantErr     bool
		wantErrType string // "ErrFieldMismatch" or empty
	}{
		{
			desc: "partial struct with extra fields - should pass (partial object support)",
			src: &pb.Entity{
				Key: keyToProto(testKey0),
				Properties: map[string]*pb.Value{
					"Name":    {ValueType: &pb.Value_StringValue{StringValue: "John"}},
			...
			dst:         &PartialStruct{},
			want:        &PartialStruct{Name: "John", Age: 30, Email: "john@example.com"},
			wantErr:     false, // Should pass - extra fields are now skipped
			wantErrType: "",
		},
		{
			desc: "full struct matching all fields - should pass (positive test)",
			src: &pb.Entity{
				Key: keyToProto(testKey0),
				Properties: map[string]*pb.Value{
					"Name":    {ValueType: &pb.Value_StringValue{StringValue: "Jane"}},
					"Age":     {ValueType: &pb.Value_IntegerValue{IntegerValue: 25}},
					"Email":   {ValueType: &pb.Value_StringValue{StringValue: "jane@example.com"}},
					"Address": {ValueType: &pb.Value_StringValue{StringValue: "456 Oak Ave"}},
					"Phone":   {ValueType: &pb.Value_StringValue{StringValue: "555-5678"}},
					"Country": {ValueType: &pb.Value_StringValue{StringValue: "USA"}},
				},
			},
			dst: &FullStruct{},
			want: &FullStruct{
			...
			wantErr: false, // Should pass - all fields match
		},
		{
			desc: "partial struct with type mismatch on existing field - should fail",
			src: &pb.Entity{
				Key: keyToProto(testKey0),
				Properties: map[string]*pb.Value{
					"Name": {ValueType: &pb.Value_StringValue{StringValue: "John"}},
					"Age":  {ValueType: &pb.Value_StringValue{StringValue: "thirty"}}, // Wrong type - should be int64
				},
			},
			dst:         &PartialStruct{},
			want:        &PartialStruct{Name: "John"},
			wantErr:     true, // Should fail - type mismatch
			wantErrType: "ErrFieldMismatch",
		},
		{
			desc: "partial struct with nested extra fields - should pass (partial object support)",
			src: &pb.Entity{
				Key: keyToProto(testKey0),
				Properties: map[string]*pb.Value{
					"Name": {ValueType: &pb.Value_StringValue{StringValue: "John"}},
					"Age":  {ValueType: &pb.Value_IntegerValue{IntegerValue: 30}},
					"Extra": {ValueType: &pb.Value_EntityValue{
						EntityValue: &pb.Entity{
							Properties: map[string]*pb.Value{
								"Field1": {ValueType: &pb.Value_StringValue{StringValue: "value1"}},
								"Field2": {ValueType: &pb.Value_StringValue{StringValue: "value2"}},
							},
						},
					}}, // Extra nested field - should be skipped
				},
			},
			dst:         &PartialStruct{},
			want:        &PartialStruct{Name: "John", Age: 30},
			wantErr:     false, // Should pass - extra nested field is now skipped
			wantErrType: "",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				if tc.wantErrType == "ErrFieldMismatch" {
					if _, ok := err.(*ErrFieldMismatch); !ok {
						t.Errorf("expected ErrFieldMismatch, got %T: %v", err, err)
						return
					}
				}
			} else {
