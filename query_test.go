...
func TestInvalidFilters(t *testing.T) {
	...
	// Test invalid filter (empty field name) - should still error
	q1 := NewQuery("SomeKey").Filter("", 0)
	if _, err := client.Count(context.Background(), q1); err == nil {
		t.Errorf("%+v: got nil, wanted error for empty filter", q1)
	}

	// Test custom type - should now work with auto-casting (no error expected)
	type MyType int
	var v MyType = 1
	q2 := NewQuery("SomeKey").Filter("fld=", v)
	if _, err := client.Count(context.Background(), q2); err != nil {
		// With auto-casting, custom types should work
		// Only error if there's a query construction error, not a type error
		if q2.err == nil {
			t.Errorf("%+v: unexpected error with auto-casting: %v", q2, err)
		}
	}
}
...
// TestGetAllWithUnparsedFields tests the GetAllWithUnparsedFields function.
// This function should return partial matches + information about unparsed fields.
func TestGetAllWithUnparsedFields(t *testing.T) {
	// Define a partial struct (subset of fields)
	type PartialGopher struct {
		Name string
	}

	// Define a full struct (all fields)
	type FullGopher struct {
		Name   string
		Height int64
		Weight float64
		Active bool
						...
						EntityResults: []*pb.EntityResult{
							{
								Entity: &pb.Entity{
									...
									Properties: map[string]*pb.Value{
										"Name":   {ValueType: &pb.Value_StringValue{StringValue: "George"}},
										"Height": {ValueType: &pb.Value_IntegerValue{IntegerValue: 32}},
										"Weight": {ValueType: &pb.Value_DoubleValue{DoubleValue: 150.5}},
										"Active": {ValueType: &pb.Value_BooleanValue{BooleanValue: true}},
										"Extra":  {ValueType: &pb.Value_StringValue{StringValue: "extra_field"}}, // Unparsed
									},
								},
							},
							{
	...
	t.Run("partial struct - should return unparsed fields", func(t *testing.T) {
		var dst []PartialGopher
		// This should fail initially - function doesn't exist
		keys, unparsed, err := client.GetAllWithUnparsedFields(ctx, NewQuery("Gopher"), &dst)
		if err == nil {
			// If function exists, verify behavior
			if len(keys) != 2 {
				t.Errorf("expected 2 keys, got %d", len(keys))
			}
			if len(dst) != 2 {
				t.Errorf("expected 2 entities, got %d", len(dst))
			}
			// Check unparsed fields - should contain Extra, Extra1, Extra2, Height, Weight, Active
			expectedUnparsed := map[string]string{
				"Extra":  "string",
				"Extra1": "string",
				"Extra2": "int64",
				"Height": "int64",
			...
			if len(unparsed) != len(expectedUnparsed) {
				t.Errorf("expected %d unparsed fields, got %d: %v", len(expectedUnparsed), len(unparsed), unparsed)
			}
			for field, expectedType := range expectedUnparsed {
				if actualType, ok := unparsed[field]; !ok {
					t.Errorf("expected unparsed field %q with type %q, but field not found", field, expectedType)
				} else if actualType != expectedType {
					t.Errorf("expected unparsed field %q to have type %q, got %q", field, expectedType, actualType)
				}
			}
		} else {
			// Function doesn't exist yet - this is expected for baseline
			t.Logf("Function doesn't exist yet (expected for baseline): %v", err)
		}
	})

	t.Run("full struct - should return empty unparsed map", func(t *testing.T) {
		// Create a client with data that matches FullGopher struct exactly
		fullClient := &Client{
			client: &fakeClient{
				queryFn: func(req *pb.RunQueryRequest) (*pb.RunQueryResponse, error) {
					return &pb.RunQueryResponse{
		...
		var dst []FullGopher
		keys, unparsed, err := fullClient.GetAllWithUnparsedFields(ctx, NewQuery("Gopher"), &dst)
		if err != nil {
		...
		if len(unparsed) != 0 {
			t.Errorf("expected empty unparsed map for full struct, got %d fields: %v", len(unparsed), unparsed)
		}
	})
}

// TestCountFunction tests the Count function with various scenarios.
// These tests should pass with current implementation, then continue to pass after GQL replacement.
func TestCountFunction(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			query:   NewQuery("Gopher"),
			want:    2,
			wantErr: false,
		},
		{
			desc: "count with filters",
			setupFn: func() *Client {
				return &Client{
					client: &fakeClient{
						aggQueryFn: func(req *pb.RunAggregationQueryRequest) (*pb.RunAggregationQueryResponse, error) {
			...
			query:   NewQuery("Gopher").FilterField("Name", "=", "George"),
			want:    1,
			wantErr: false,
		},
		{
			desc: "count with empty results",
			setupFn: func() *Client {
				return &Client{
					client: &fakeClient{
						aggQueryFn: func(req *pb.RunAggregationQueryRequest) (*pb.RunAggregationQueryResponse, error) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			client := tc.setupFn()
			count, err := client.Count(ctx, tc.query)
			if tc.wantErr {
				if err == nil {
			...
			if count != tc.want {
				t.Errorf("got count %d, want %d", count, tc.want)
			}
		})
	}
}

// TestFilterFieldAutoCasting tests that FilterField automatically casts enums and custom types.
func TestFilterFieldAutoCasting(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:     "enum type single value - should pass (auto-casting works)",
			field:    "Status",
			operator: "=",
			value:    StatusActive,
			wantErr:  false, // Should pass - auto-casting handles it
		},
		{
			desc:     "custom int type - should pass (auto-casting works)",
			field:    "UserID",
			operator: "=",
			value:    UserID(123),
			wantErr:  false, // Should pass - auto-casting handles it
		...
		{
			desc:     "custom float type - should pass (auto-casting works)",
			field:    "Price",
			operator: "=",
			value:    Price(99.99),
			wantErr:  false, // Should pass - auto-casting handles it
		...
		{
			desc:     "enum array for 'in' operator - should pass (auto-casting works)",
			field:    "Status",
			operator: "in",
			value:    []Status{StatusActive, StatusInactive},
			wantErr:  false, // Should pass - auto-casting handles array elements
		},
		{
			desc:     "custom int array for 'in' operator - should pass (auto-casting works)",
			field:    "UserID",
			operator: "in",
			value:    []UserID{UserID(1), UserID(2), UserID(3)},
			wantErr:  false, // Should pass - auto-casting handles array elements
		},
		{
			desc:     "primary type int64 - should pass (positive test)",
			field:    "Age",
			operator: "=",
			value:    int64(30),
			wantErr:  false, // Should pass - primary type
		},
		{
			desc:     "primary type string - should pass (positive test)",
			field:    "Name",
			operator: "=",
			value:    "John",
			wantErr:  false, // Should pass - primary type
		...
		{
			desc:     "primary type array for 'in' operator - should pass (positive test)",
			field:    "Age",
			operator: "in",
			value:    []interface{}{int64(1), int64(2), int64(3)},
			wantErr:  false, // Should pass - primary type array
		},
	}

	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			q := NewQuery("TestKind").FilterField(tc.field, tc.operator, tc.value)
			// Check if FilterField itself has an error
			if q.err != nil {
				if !tc.wantErr {
					t.Errorf("unexpected FilterField error for %s %s %v: %v", tc.field, tc.operator, tc.value, q.err)
				}
				return
			}
			// Try to convert to proto to trigger the actual validation
			// This is where custom types would fail without auto-casting
			if len(q.filter) > 0 {
				if pf, ok := q.filter[0].(PropertyFilter); ok {
					_, err := pf.toProto()
					if tc.wantErr {
						if err == nil {
							t.Errorf("expected proto conversion error for %s %s %v, but got none", tc.field, tc.operator, tc.value)
						}
					} else {
						if err != nil {
							t.Errorf("unexpected proto conversion error for %s %s %v: %v", tc.field, tc.operator, tc.value, err)
						}
					}
				}
			}
