...
// autoCastFilterValue automatically casts custom types (enums, custom int/float types) to base types.
// This allows FilterField to accept custom types without requiring manual casting.
func autoCastFilterValue(v interface{}) interface{} {
	if v == nil {
		return nil
	}
	...
	if rv.Kind() == reflect.Slice || rv.Kind() == reflect.Array {
		...
		for i := 0; i < length; i++ {
			result[i] = autoCastFilterValue(rv.Index(i).Interface())
		}
		return result
	}
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		// Custom int types (including enums with underlying int) -> int64
		return rv.Int()
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		// Custom uint types -> int64 (datastore uses int64)
		return int64(rv.Uint())
	case reflect.Float32, reflect.Float64:
		// Custom float types -> float64
		return rv.Float()
	case reflect.Bool:
		// Custom bool types -> bool
		return rv.Bool()
	case reflect.String:
		// Custom string types -> string
		return rv.String()
	default:
		// For other types (pointers, structs, etc.), return as-is
		// They will be handled by interfaceToProto or will error appropriately
		return v
	}
}
...
//
// This implementation uses GQL aggregation queries for efficient server-side counting.
// The running time and number of API calls are constant regardless of result set size.
//
...
func (c *Client) Count(ctx context.Context, q *Query) (n int, err error) {
	...
	// Use aggregation query with count for efficient server-side counting
	aq := q.NewAggregationQuery()
	// Use explicit "count" alias to ensure we can find it
	countAlias := "count"
	aq.WithCount(countAlias)

	// Run the aggregation query
	ar, err := c.RunAggregationQuery(ctx, aq)
	if err != nil {
		return 0, err
	}

	// Extract count from aggregation result
	countValue, ok := ar[countAlias]
	if !ok {
		// Try default alias format as fallback (WithCount generates "{kind}_count" if empty)
		if q.kind != "" {
			defaultAlias := fmt.Sprintf("%s_%s", "count", q.kind)
			countValue, ok = ar[defaultAlias]
		}
		if !ok {
			return 0, fmt.Errorf("datastore: count result not found in aggregation response")
		}
	}

	// Convert count value to int
	// AggregationResult values are *pb.Value types
	if pbValue, ok := countValue.(*pb.Value); ok {
		// Check if it's an IntegerValue type
		if _, ok := pbValue.ValueType.(*pb.Value_IntegerValue); ok {
			iv := pbValue.GetIntegerValue()
			return int(iv), nil
		}
		return 0, fmt.Errorf("datastore: invalid count value type, expected IntegerValue, got %T", pbValue.ValueType)
	}
	return 0, fmt.Errorf("datastore: unexpected count value type: %T", countValue)
}
...
// GetAllWithUnparsedFields runs the provided query and returns all keys that match,
// appends the values to dst, and returns information about fields that couldn't be parsed.
// The unparsed map contains property names as keys and their type names as values.
func (c *Client) GetAllWithUnparsedFields(ctx context.Context, q *Query, dst interface{}) (keys []*Key, unparsed map[string]string, err error) {
	ctx = trace.StartSpan(ctx, "cloud.google.com/go/datastore.Query.GetAllWithUnparsedFields")
	defer func() { trace.EndSpan(ctx, err) }()
	...
	if !q.keysOnly {
		dv = reflect.ValueOf(dst)
		if dv.Kind() != reflect.Ptr || dv.IsNil() {
			return nil, nil, ErrInvalidEntityType
		}
		dv = dv.Elem()
		mat, elemType = checkMultiArg(dv)
		if mat == multiArgTypeInvalid || mat == multiArgTypeInterface {
	...
	for t := c.Run(ctx, q); ; {
		...
		if !q.keysOnly {
			...
			// Convert pb.Entity to Entity for loading
			ent, err := protoToEntity(e)
			if err != nil {
				return nil, nil, err
			}
			// Load with unparsed tracking
			entityUnparsed, err := loadEntityToStructWithUnparsed(ev.Interface(), ent)
			if err != nil {
				if _, ok := err.(*ErrFieldMismatch); ok {
					errFieldMismatch = err
				} else {
			...
			// Merge unparsed fields from this entity into the overall map
			for name, typ := range entityUnparsed {
				unparsed[name] = typ
			}
			if mat != multiArgTypeStructPtr {
				ev = ev.Elem()
	...
	return keys, unparsed, c.processFieldMismatchError(errFieldMismatch)
}
